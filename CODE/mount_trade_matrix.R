...
raw <- data.frame(read.table("../DATA/EPI_DATA/database_trade.txt", header = TRUE))
...
totals <- data.frame(FROM, TO, Cattle, Sheep, Pig)
write.table(totals, file = "../DATA//EPI_DATA//trade_totals.txt" , row.names = FALSE)
#
#Cattle=1
#Sheep=2
#Pig=3
...
trade <- data.frame(grid, cattle, sheep, pig)
write.table(trade, file = "../DATA//EPI_DATA//TRADE.txt", sep = "\t", row.names = FALSE)
