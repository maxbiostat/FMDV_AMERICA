...
%%%%%
%opening
\title{Text S2 -- Supplementary information to ``Spatiotemporal Dynamics of Foot-and-Mouth Disease Virus in South America''} % aargh, lousy name
\author{
...
While for large data sets it is possible to perform equal down-sampling, i.e., simple random stratified sampling~\cite{M-fluPNAS}, this was not posssible  with the data sets analyzed in this study.
Therefore, to further assess the effect of biased sampling while still using the information from overrepresented locations, we used the down-sampling scheme detailed below.
...
Another point of interest is to assess whether the estimates of $\mathbf{Q}$ were consistent across subsamples.
...
[COMMENT ON PERCENTUAL DISCREPANCIES]
...
From these we can see that divergence between subsamples was ver
...
\textbf{Figure~\ref{sfig:trade}. Production time series for pigs, cattle and sheep in South America.}
We show og~(~\#~of heads) of live animals for pigs, sheep and cattle.
...
We show the origins of serotype A strains in Bolivia (A) and Brazil (B) and Uruguay (C) in $2001$ and Colombia in $2008$ (D).
