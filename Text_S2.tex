...
%GB: I don't think we need this statement as all this stuff will/should be hosted on the journal's website.
%LM: done
...
PS is defined simply as the number of as the number of character state changes in the tree~\cite{Fitch1971}.
...
\end{equation}
where $f_i$ is the proportion (frequency) of the most common trait value among the terminal branches (tips) subtended by node $i$ and $m_i$ is the number of tips subtended by node $i$.
Clearly, the lower the AI the stronger the phylogeny–trait association.
Now let $\mathbb{I}(i) = 1$ if  \textbf{all} tips subtended by node $i$ have the same trait value $t$ and $\mathbb{I}(i) = 0$ otherwise.
...
The problem of determining if the statistical relevance of the observed values for these indexes, however, remains.
...
Results indicate that while the best model fit for serotype A is provided by a log-normal molecular clock, an exponential relaxed molecular clock offers a better model fit for serotype O, pointing to different evolutionary dynamics between the two serotypes with respect to branch-specific rate variation. 

\section*{Livestock trade data}

One of the main contributions of this paper is to explore the association between cattle, pigs and sheep trade and viral diffusion using a statistically sound approach that enables us to test evolutionary hypotheses concerning the influence of these predictors.
...
Spatially, the trade of all three livestock is rather differening, as shown in Figure~\ref{sfig:tradenets}.
...
%GB: are you sure this is really called preferential sampling? I thought that was something different ...
...
%GB: do we have a citation for this? Perhaps the consequence of an unbalanced design can be explained in more detail here?
...
While for large data sets it is possible to perform equal down-sampling, i.e. simple random stratified sampling~\cite{M-fluPNAS}, this was not possible with the data sets analyzed in this study. %GB: because ...
...
%GB: explain clearly what is seen in all the figures and tables, as well as what can be concluded from them. This needs to be in the corresponding legends.
...
Note also the long range trade routes of sheep between Argentina and Colombia, which are absent from the pig network.
...
The origins of the isolates from Uruguay $2001$, however, could not be traced to a particular location.
This may indicate that Uruguay experienced introductions from both Brazil and Argentina.
...
The more flexible skyride tree prior clearly outperforms the assumption of constant population size, what is consistent with the fact that FMDV (both serotypes) has experienced periods of population expansion, i.e., transmission within the time frame considered.
