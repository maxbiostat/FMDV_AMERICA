...
Results of these experiments are presented in Tables~\ref{tab:ED_A} and~\ref{tab:ED_O}.
...
Another way of looking into this matter is to see $\mathbf{Q}$ as whole matrix (instead of entry-wise).
To address this we calculated the $L_1$ and $L_2$ matrix distance norms for (each pair of) the estimates of $\mathbf{Q}$ obtained from each subsample.
...
\end{equation}
and the $L_2$ norm has the form
\begin{equation}
 L_2 = \sqrt{\frac{1}{K^{2} (\sum_{i=1}^{K^2} (X_i-Y_i)^2 )}}
\end{equation}
...
Panels C and D show heat maps of the $5\times5$ matrices of $L_1$ norms comparing each subsample, for serotypes A and O, respectively.
