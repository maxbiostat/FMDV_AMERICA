...
% Overall, the inferred MCC trees suggest that most FMDV flow within South America occurred between neighbouring countries. % LM: Maybe superfluous
...
In the serotype A phylogeography, our data suggests that Brazil is connected to Argentina, Uruguay, Venezuela and Colombia.
For serotype O, Colombia was connected to Venezuela, Ecuador and Bolivia.
...
The presence of Bolivia as a viral hub for serotype O dispersal and its negligible role for serotype A  dispersal is an interesting difference between the two serotypes. % LM: removed 'striking'. Keep in mind that there is no reason whatsoever for the two serotypes to have different spatial patterns. But still, they seem to.
Likewise, Brazil is highly connected in the serotype A network but not on the serotype O network.

% For serotype A, the highest support (BF~$>800$) was observed for viral flow from Venezuela to Colombia and from Argentina to Uruguay. % LM: Already presented.
...
% Assuming there is no preference towards spread through borders would give a prior expectation on the proportion $B/(B + \text{NB})\approx 0.5$ for both serotypes. % LM: Confusing so I took it out. If a referee asks for it my Wrath will be unleashed, though.
% Not entirely surprisingly, the posterior proportion of viral migration events occurring through borders indicate that most FMDV transmission occurs through shared borders for both serotypes. % LM: superfluous.
...
% Intriguingly, the results of this analysis (Table~\ref{tab:preds}) show different best fitting predictors for each serotype. %LM: superfluous/not appropriate here.
...
We calculated log Bayes factors and observed good agreement between sub-samples in terms of supported routes for both serotypes (see Figures~\ref{sfig:bssvsA} and~\ref{sfig:bssvsO}). %LM: notice that we do not ``estimate'' the BF here. HERE we CALCULATE it. this is no approximation, it's analytical. If you're confident the run has converged and the inclusion probability is correct, of course.
...
Moreover, serotype-specific disease notification (number of cases) data was overlaid on the demographic reconstruction to illustrate the relationship between viral diversity and the onset of epidemics. 
...
Our results suggest, this serotype O lineage was introduced in Ecuador from Colombia (Figure~\ref{sfig:epitrac}E) and then underwent endemic circulation.
...
We apply the non-parametric skyride coalescent model~\cite{skyride}% LM: Nuno wants to know why not skyride. My answer: because when I started this project, skygrid wasn't even out yet ;0)
in order to reconstruct the past population dynamics for both serotypes, using a Gaussian Markov Random Field (GMRF) prior to obtain smooth estimates for effective population size trajectories over time.
...
For these analyses we used a sample of $10,000$ trees from the posterior distribution of topologies.%LM: (after removal of 10\% burn-in). You see? This is just wrong: techinically, you remove the burn-in because it is NOT a part of the posterior.
