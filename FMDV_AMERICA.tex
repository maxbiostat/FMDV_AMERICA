...
\section*{Tables}
\begin{table}[!ht]
\caption{{\bf Comparison of combined and separate models for CTMC rate matrix}. Using data from both serotypes jointly, we estimated marginal likelihoods for a model in which a single rate matrix was estimated (``Shared'') and one in which two separate matrices were estimated (``Separate''). We present results obtaining by collecting different numbers of samples per ``step'' in Path Sampling (PS) and Stepping Stone Sampling (SS) algorithms.}
 \begin{center}
