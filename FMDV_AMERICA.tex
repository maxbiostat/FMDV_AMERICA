...
\myedit{ThisIsAnEdit}{Create the edits in your text like this.}
...
% Root-to-tip plots against sampling times from maximum likelihood phylogenies constructed with PhyML~\citep{Guindon2003} suggest a linear trend (Figure~\ref{sfig:root-to-tip}), which encourages the pursuit of more detailed estimates.
% In addition, we compare a model with and without dated-tips (the `temporal signal' test~\citep{Faria2012, Baele2012}, see Methods) and find significant temporal structure for both serotypes (A: log BF = $310$; O: log BF = $348$), showing that sufficient temporal information is embedded in the sequence data of both serotypes under investigation, which is essential for the estimation of divergence times and the reconstruction of the population dynamics in natural time units~\citep{Drummond2003}. 
% 
...
% The estimated evolutionary rate for serotype O is approximately 2.5 times faster than serotype A, i.e.~at $\approx 1 \times 10^{-2}$, consistent with previous studies~\citep{Tully2008, Carvalho2013, Muellner2011}.
...
% We further explore this using Bayesian Tip-Significance testing (BaTS)~\citep{Parker2008} to calculate phylogeny-trait association indices such as the parsimony score (PS), association index (AI) and maximum clade size (MC) (see Text S2 for details).
...
% 
% To gain insight into the spatio-temporal process of FMDV spread, we employ an asymmetric continuous-time Markov chain (CTMC) phylogeographic model~\citep{Lemey2009} implemented in BEAST~\citep{Drummond2012}, coupled with model averaging using a Bayesian stochastic search variable selection (BSSVS) procedure.
...
% 
% We subsequently turn our attention to the statistical support for epidemiological linkage between pairs of locations using Bayes factors~\citep{Lemey2009}.
...
% \end{center}
% 
% To infer which countries have most significantly contributed to the dispersal of FMDV serotypes, we estimate net migration flow (efflux - influx) using a robust counting approach~\citep{Minin2008b}.
...
% We use geographic distances as well as the data on the trade of livestock to specify CMTC rate matrix priors (potential predictors), and we employ PS/SS to calculate (log) marginal likelihoods for each predictor to determine the importance of each variable for viral spread~\citep{Carvalho2013, Nelson2011}.
...
% Regarding spatial signal extraction, as measured by the Kullback-Leibler (KL) divergence (relative  to a uniform prior) at the root~\citep{Lemey2009} (see also Text S2) for each predictor, we find KL divergences ranging from $3.86$ to $5.91$, showing highly concentrated distributions at the root. 
...
% 
% Since our data sets contain unbalanced geographic samples (Table~\ref{stab:reps}), we conduct a detailed sensitivity analysis to assess the robustness of our results to over-representation of some locations~\citep{Faria2012, Bahl2011, Bedford2010, Edwards2011}.
...
% Another aspect of interest is the amount of information extracted from each sample, which we measure by calculating Kullback-Leibler~\citep{Kullback1951} divergence between prior and posterior distributions of spatial location at root~\citep{Lemey2009}.
...
% These differences in evolutionary rate magnitude and variability suggest that, although the two serotypes share the same host range and infection routes, they present rather different evolutionary dynamics in the continent, which may help explaining their different emergence patterns (see Figure 3 in Naranjo \& Cosivi, 2013~\citep{Naranjo2013}). 
% 
...
% This separation suggests that viral transmission may follow different regimes in these two regions, which have different forms of livestock production~\citep{Saraiva2003,Naranjo2013}.
...
% Since both Peru, Bolivia and Venezuela are considered to have achieved less than expected regarding the implementation of control measures~\citep{Naranjo2013}, a link passing through these countries is plausible.
...
% It should be stressed however that while Colombia has taken the appropriate steps to controlling FMD in its territory, Venezuela still faces challenges in implementing the necessary control policies~\citep{Naranjo2013}.
% 
...
% The trade of cattle is the most significant predictor for serotype O spread, which is compatible with the notion that these hosts are the most important for FMDV maintenance and transmission, even though sheep population sizes are on par with those of cattle~\citep{Saraiva2003}.
...
% 
% Previous studies have shown that occurrences caused by serotype A present longer cycles and wider epidemics, while serotype O is more prevalent with shorter disease cycles~\citep{Gallego2007}.
% These epidemiological features are reflected in the temporal variation observed for viral $Ne$ in both serotypes, a result obtained for other viruses as well~\citep{Bennett2010,Pybus2003}. 
...
% Despite these important differences, from the temporal reconstructions for both serotypes it can be deduced that over time, with the increase of vaccination coverage, viral effective population size ($Ne$) decreases dramatically, a result previously obtained for serotype O in Ecuador~\citep{Carvalho2013}.
...
% The figure presents spatial projections of the maximum clade credibility trees produced with SPREAD~\citep{Bielejec2011} and visualized using Google Earth (\url{http://www.google.com/earth/index.html}).
...
% We estimate the number of migration events between countries using the robust counting (Markov jumps) techinique of~\citep{Minin2008b}.
...
% We use the Kullback-Leibler divergence of the posterior distribution at root to a uniform distribution as a measure of spatial signal extraction~\citep{Lemey2009}.
