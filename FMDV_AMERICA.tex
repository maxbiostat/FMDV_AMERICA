...
%LM: agreed. Omitted the  model entirely. I'll state which one we used in the Methods section.
...
For serotype O there is also interspersing of clades from different locations, with  one large clade featuring interleaved Ecuadorian and Colombian clades (Figure~\ref{fig:trees}B). %LM: again, should we indicate this? I think Nuno is the guy here [he also made the plots].
...
%LM: Duno exactly what you mean, but I'll add some detail.
...
%LM: The main point is to show that I'm not waving my arms around talking about 'interspersing'. Rather, there is ''statistical support'' for the claim that there is evidence for migration in the tree(s). Added a section detailing this in Text S2 as well.

%GB: what do you mean when you say 'richer' ?
%LM: *I* don't mean anything: Marc wrote that line. :0)
...
%LM: Done. Also collapsed and re-arranged some paragraphs to tighten up a bit.
...
%LM: good point. Modified accordingly.
...
Transitions from Brazil to Bolivia and Peru and Ecuador (bidirectional) also yielded considerable statistical support (BF~$>10$). % LM: Note these are Bayes factors rather than log-Bayes factors
...
%LM: you're absolutely right, I must have been on dope when I wrote that... o.O Decided to  re-do it completely.
...
%LM: I will have to beg to strongly disagree here: Biologists are dumb people [yeah I am one and so is Andrew and he's probably gonna kick my ass for saying this, but whatever ;0)] they can't read tables and need pretty figures for everything.
%LM: The problem with charges will only arise if we choose to submit to a journal that has print versions. BTW, where are we going to submit?? 
...
%LM: you're right. I deliberately chose NOT to make the figures self-contained. But I can see why that may annoy people, so I'll modify accordingly. But the statements below are quite fine, IMHO. Theres not much more to say/show here.
...
%LM: I really dont know. I'd say we keep it, just to have a baseline to compare to. But I'm interested in Philippe's/Andrew's take on the matter.
...
%LM: Simply because the objective here is to see what happens with the distribution at root (the \pi vector of the CTMC) for various predictors. I try to motivate and clarity a bit more below.
...
%LM: good point. It actually can be quite useful, but not as a detached two-line paragraph. Fixed.
...
%LM: you have a point, should be better explained. But come on man, cut me some slack: it is clearly said that we draw FIVE  sub-samples... ;0)
...
%LM; No need, I explain better below. 
...
%LM: I disagree, but whatever, I'll change it.
...
%Yep. Solution: collapse results and discussion, interspersing some of the statements below into the results and have a shorter section with concluding remarks wrapping it all up
...
%LM: Not particularly, I guess. added a couple words though.
...
%LM: I sort of agree with you. But now it's here and would be worse to get rid of it than  to fix it. Not gonna happen in the future... XDD. Also, I don't get you: I spend 3 lines explaining what BaTS is doing, man! What else could I say?
%LM: I'll expand a little more on Text S2, but against my will, since I think there is enough detail already.
...
%LM Good point. Fixed it to be more explicit.
...
%LM: Again, I beg to differ: the neat thing of this paper is how we use the marg likes for spatial model selection. Since we need to explain this in detail, we might as well do it here. Note that above there is a note to refer to this section for details on the model selection.
...
%LM True. Hope to have made it clearer now.
...
The numbers of live animals exchanged between countries were normalized to have mean and coefficient of variation of $1$ and used as prior expectations in BEAST (see~\cite{roots}, pag. 14 and the Appendix in~\cite{Carvalho2013}).  
...
% As stated above, you're right. Against my will I will create more informative figure legends.
...
%LM: I'll see if I can fix it. Gonna be a pain though.
