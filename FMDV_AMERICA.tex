...
Moreover, from the separate and shared rate matrix analysis we found that rates inferred for serotype A are more strongly correlated with the rates estimated under a shared rate matrix.
However, Bayes factors assessment of the statistical support for the two hypotheses (shared or separate rate matrices), indicates that a shared model should be preferred to a model with separate rate matrices for each serotype (BF$>10$, see Table~\ref{tab:sharedsep}).
...
% Our results suggest that FMDV serotypes have different dispersal determinants,  (plots and all) <- conflicting information
