...
[REVISE RESULTS]
% The most probable origin of the strains isolated in Argentina $2001$ (Figure~\ref{fig:epidemictracing}A) Brazil (posterior probability = $0.76$) while Uruguay received $0.1$ posterior probability.
% Results for the Brazilian strains of the same year point to Uruguay as most probable source as shown in Figure~\ref{fig:epidemictracing}B.
% All trees in the posterior distribution analyzed showed multiple origins for the two sequences from  Uruguay in $2001$ (data not shown).
% Interestingly, the origin of serotype O strains from Colombia in $1994$ was found to be the root of the phylogenies with high posterior probability ($0.76$).
% The probability that the $1994$ strains were already circulating in Colombia was $~0.20$ suggesting this hypothesis should not be ruled out.
% We trace the origins of the Venezuelan strains to Colombia (posterior probability $=0.61$) although the possibility of multiple sources also received non-negligible support.
% The origins for serotype A in Bolivia $2001$ are most likely to lie in Bolivia itself, suggesting the virus was already circulating there at the time (Figure~\ref{sfig:epitrac}A).
% Figure~\ref{sfig:epitrac}B shows the same pattern for serotype O in Venezuela $2003$, for which the most likely scenario was previous circulation. 
% However we note that there is a non-negligible probability that Colombia is the origin of the $2003$ Venezuelan strains (posterior probability $=0.31$).
...
These data were expressed as doses per head, which we consider to be a more accurate measure of vaccination coverage, since it corrects for population size increase/decrease over time (see Figure ~\ref{sfig:trade} for livestock population time series). 
