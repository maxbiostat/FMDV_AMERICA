...
We further explore this using the Bayesian Tip-Significance testing (BaTS)~\cite{bats} and show significant location-tip association for both serotypes (Supplementary Information, Table ~\ref{stab:BaTS}).
...
%LM: The main point is to show that I'm not waving my arms around talking about 'interspersing'. Rather, there is ''statistical support'' for the claim that there is spatial structure in the tree(s).
...
To further explore the spatial behavior of FMDV  we used robust counting~\cite{Minin2008} to compute the posterior expected number of location-state transitions along the branches of the phylogeny and provide a quantitative measure of viral flow between countries.
...
%LM: you're absolutely right, I must have been on dope when I wrote that. Decided to  re-do it completely.
...
However, the importance of long-range migration routes seems to differ for both serotypes, since the posterior median proportion of ``non-border'' transitions is considerably different (A: $0.14$, $0.00$--$0.28$  and O: $0.05$, $0.00$--$0.20$).

%GB: I would expand some more on the sentences below, as this is quite an interesting result
[REWRITE BELOW]

The net migration flow (efflux - influx) for each country, along with the Bayes factors from a separate BSSVS analysis show that supported migration routes differ for each serotype, with some overlapping routes connecting Venezuela, Colombia and Ecuador(Figure~\ref{fig:mj&BFs}). 
When we analyzed the net migration rate  for each country using the MJ counts, we noticed that the pattern of emitters/receivers is different for each serotype.
